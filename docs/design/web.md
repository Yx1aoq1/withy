...
> 适用范围:`packages/app`(`@withy/app`,Next.js 16 App Router + React 19 + Tailwind 4)。
> 定位:实施规格级。dashboard 是**多项目管理器 + 全局配置台**,数据读写全部经 [@withy/core](./core.md),浏览器不碰 fs。
...
1. **浏览器不直接碰 fs**。所有 `.withy/` 读写经 Next route handler(Node runtime)→ `@withy/core`。
2. **不重写 core 逻辑**。门禁/读取/状态机调用 core 的同一份实现,与 CLI 行为一致。

```text
Browser ──HTTP──► Next route handler(node runtime)──► @withy/core ──► .withy/
   ▲                        │
...
```text
            ...
            └───────────┬──────────────────┘
        全局视图        │        项目视图(可切换)
   ~/.withy/config     │     <repoA>/.withy/   <repoB>/.withy/
   ~/.withy/projects   │     task 看板          task 看板
   ~/.withy/knowledge  │     context/knowledge  context/knowledge
...
```text
加项目按钮 → 选目录路径 → core.detectWithy(path)
   ├─ 有 .withy/ → upsert 进 ~/.withy/projects.json → 出现在项目列表
   └─ 无 .withy/ → 提示未初始化,展示「初始化项目」按钮(§2.4)
...
| 视图     | scope           | 用户过滤                           |
| -------- | --------------- | ---------------------------------- |
| 全局配置 | `~/.withy`      | 不过滤(`shouldFilterByUser=false`) |
| 项目看板 | `<repo>/.withy` | 按 `.developer` 过滤 mine/all      |

### 2.3 项目根传递

旧设计靠 `withy dashboard start` 传单一 `WITHY_PROJECT_ROOT`。新设计:dashboard 启动只需能定位 `~/.withy`;**具体项目由前端选择,经 API `project` 参数传入**,不再由启动环境固定。`WITHY_PROJECT_ROOT` 退化为「无选择时的默认项目」兜底。

### 2.4 web 触发 init(可行,非权限问题)

dashboard 以**启动它的用户身份**运行,route handler `spawn('withy', ['init', ...])` 对该用户可访问的目录有读写权,**无需提权,不存在权限问题**。会失败的只有「目录只读 / 属他人」这类边角,等同 CLI 手动 init 的报错。真正要解决的是 **init 的交互性**与**路径安全**:

```text
「初始化项目」按钮(无 .withy/ 时显示)
  ...
  → 校验:path 存在 / 是目录 / 无 .withy/(已存在则拒)→ 用户确认
  → serializeToCommand(config) → spawn('withy', ['init','--codex','--claude','-u','yan'], { cwd: path })
...
项目(?project=<path>)
  ...
  /p/tasks/<id>    详情:三层进度(§3.4)、artifact、归档按钮(仅「已完成」任务;未完成任务标 cancelled 经 CLI `withy archive --cancelled`)
    ├ 事件时间线(events.jsonl):验收尝试/跳步/跳过 + planned vs injected 注入对比 ← 发现 hook 失效的关键视图
    ├ approval 面板(展示 + 可选补批;CLI/agent 经 `withy approve` 亦可写,§6)
    ├ 实施计划(implement.md):只读渲染,best-effort 解析复选框算 done/total + 标未识别行数(§3.4、core §4.7)
...
**`summary.ts` 自实现了任务读取、`product.ts` 抄了常量 —— 本轮要全部替换为 `@withy/core` 调用**(§5)。
...
② 节点门禁(中)  grill-me:规划产物 3/3 ✓ · approval ⚠待批
                 check:   npm test ✗(第 2 次,阈值 3)  ← gate 现算 + events 重试计数
③ 实施计划(细)  [x] 错误提示  [ ] 锁定 5 分钟  (1/2)   ← implement.md,只读渲染
```
...
| 实施计划 | `implement.md`(core §4.7)                      | `implementationProgress`                         | 逐条实施步骤的 done/未done,只读     |

- 实施计划是 **agent 维护的 markdown(`implement.md`),web 只读展示**:`implementationProgress` best-effort 扫 `- [ ]`/`- [x]` 算 done/total,并把未识别成复选框的行数一并标出(不静默丢项,core §4.7)。**web 不写回**(不提供勾选 → 无 `PUT`),勾选/改项由 agent 或人直接编辑文件,经 SSE 局部刷新(§4.2)。
...
- **只读详情(回看执行历史)**:复用活跃详情的三层展示但只读——主体阶段步进器、归档时所在节点、实施计划(只读)+ 完成度,再加生命周期(创建/完成/归档时间)与负责人;无勾选、无归档按钮。这要求按 id 能读到归档任务的 state/implement:**core 的 `readTask/readState/readImplementation/readEvents` 在 live 路径缺失时回退遍历 `archive/YYYY-MM/` 桶(写入不回退,杜绝写归档),workflow 不归档故 `phaseOf` 始终可读**。完整事件时间线(events.jsonl)仍属任务详情页 W4 范畴,不塞进 336px 侧栏。
...
| `GET /api/knowledge/graph?scope&project&merged`           | 文档关系图(节点/边),`merged=1` 出全局+项目全景                                                     | `/p/knowledge` 图谱视图;调 `withy knowledge graph`(knowledge.md §9)                                          |
...
agent 在 CLI/对话里改 `.withy/`(写 artifact、`withy complete` 改 state)是**另一个进程写盘**,web 不会自动反映。用 chokidar 监听 + SSE 推送实现实时:

```text
dashboard server:chokidar.watch(<当前项目>/.withy/{tasks,workflows,context.json})
...
落地顺序:先让 app 依赖 `@withy/core`,把 `summary.ts` 改成薄封装(调 `core.listTasks` + `core.isOwnedBy`),删 `product.ts` 自定义常量(core.md K6 / cli.md C1)。

---

## 6. Approval 机制

approval 写入口有两个**等价**通道:web 点确认、或 `withy approve <node>`(agent/人均可跑,harness §2.6)。**不再要求必须经 web**——单人交互下 agent 替你跑 `withy approve` 即可;web 是可选的展示 + 另一写入口。
...
| W6  | approval 面板(展示 + 补批;`withy approve` 等价写入口,读写 `state.approvals`)                                    | P1        | harness §2.6/H11            |
...
| W7c | 知识库图谱视图(全局/项目/合并三档)+ `GET /api/knowledge/graph` + `withy knowledge graph`                        | P2        | knowledge.md §9             |
...
| W16 | 任务详情三层进度(phase/gate/implementation)+ `implement.md` 实施计划只读渲染                                    | ✅ 已实现 | core §4.7、§3.4             |
...
- ~~加项目无 .withy 时是否 web 触发 init~~ → **已定:做「初始化项目」按钮**,web 经非交互 init 触发(§2.4;无权限问题,需路径校验+用户确认)。
