...
> **落地状态**:当前代码已实现旧的 `completeNode` 与 CLI `withy complete <node>`;本轮决策是迁移为 `nextNode` / `withy next` 后删除 `complete` 命令与 `completeNode` 命名,不保留兼容入口。
...
- 内容源是**知识库**(`knowledge/`,knowledge.md):agent 维护的 wiki + index,外加 `kind:template` 产物模板(knowledge §4.1)。注入形态**由条目 `inject` 字段决定**:`index`(缺省,注 title+summary+路径,按需下钻)/ `full`(产物模板、必读短规范等,注正文)。**会话须知(guide.md)不走知识库**——它是工具文件 `.withy/guide.md`(§6.4)。
...
templates/common/skills/<base>/SKILL.md   (含 {{变量}})
   ...
   ▼
.agent/skill/withy-<base>/SKILL.md        (canonical;Codex/Gemini 直接读)
   │ symlink|copy(Claude)
   ▼
...
当前状态:workflow 类 5 个 SKILL.md(brainstorm/grill-me/dev/check/finish)+ 维护类 `withy-knowledge`(知识库维护,knowledge.md §5)正文均已落地(§9 H6)。`withy-knowledge` 不是 workflow 节点,而是按需调用的维护 skill,不进默认 workflow。(注:默认 workflow 的分诊 `triage` 是 **switch 节点、不引用 skill**,故无需 classify skill。)
...
- **停在 skill 节点**(PLANNING / IN PROGRESS)→ 注入当前节点 + 推进命令 `withy next`
...
| 扩充知识库       | 往 `knowledge/sources/` 放源 + 调 `withy-knowledge` skill(agent 维护 wiki/index) | 注入按 id 注索引(knowledge.md) | planned/actual 差异显形错误路径               |
...
- 旧门禁路径(`completeNode`:skill gate / switch `--branch`)、`advanceWorkflow`、`rewindTo`、`approveNode`、`resolveCurrentTask`、`withy hook session-start` 已落地并跑通闭环;**剩余缺口是迁移为 `withy next`/`nextNode`、删除 `complete`**,以及 per-turn / 子 agent hook、平台信封适配未做。
...
| H3a | `withy next` / `nextNode`:读取当前节点、承载唯一推进门禁、switch 无 branch 时输出分支提示,并删除 `complete`                           | P0                                             | H3                      |
...
| H11 | approval 读写(`state.approvals[currentNode]` + approval 事件,`withy approve`,§2.6;删除 approve 的 node 参数)                          | 🟡 旧 `approve <node>` 已实现;需迁移为当前节点 | web §6                  |
