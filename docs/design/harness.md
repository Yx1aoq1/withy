...
> 定位:实施规格级。Tuteur「约束层」核心 —— workflow 状态机、`next` 推进门禁、文档/提示词流转、hook 注入、用户扩展点。
...
```text
主动事件(agent 干完活)   →  ttur next               ┐
被动事件(平台会话开始)   →  ttur hook <event>      ├─→ @tuteur/core(唯一逻辑)
查询(看注入计划)        →  ttur context show       ┘
平台 hook 脚本(py/sh)   →  只转发到 ttur,不含业务
```

对照旧设计:`complete` 是 TS、`session-start.py` 自己读 `.tuteur/` —— 两套实现必漂移。现在 hook 退化成声明文件里一行 `ttur hook` 命令(无脚本),逻辑全回到 core。agent 面向推进命令统一为 `ttur next`:它不接收 nodeId,由 core 读取当前游标,降低 agent 传错参数的概率。

---

## 1. Workflow 模型(固定阶段容器 + 节点图)

静态定义 `workflows/<id>.workflow.json`(core §4.3),`state.json` 是动态游标。结构 = **三个固定阶段容器**(planning / 执行 / 收尾,不可增删)+ 容器内/容器前的两类节点:

```text
skill 节点   单出(next),入度不限   读 skill 照做,受可选 gate 约束;做完 ttur next 推进
switch 节点  单入多出(branches)     岔路口:agent 判断走哪条;走到就停,ttur next --branch;必含一个 default

entry → [switch] → skill → ... → (next:null 终点)   图必须无环(返工=停留原节点重试 §2.4;判错=rewind §3.1)
```

默认 workflow:`triage(switch) →{standard:planning框 | small:execute框 | research:finish框}→ ... → finish`。门禁 `gate`(artifacts/checks/approval)全部**可选**,产物按需(core §7、§4.3.1)。

**主体流程 vs 执行步骤(两层)**:phase 是**固定的有序骨架**(三个容器),不是自由标签:

```text
主体流程(粗粒度)= 三个固定容器   规划 → 执行 → 收尾   ← "在哪个大阶段",驱动 task.status / web 三框进度
执行步骤(细粒度)= 框内/框前节点    triage→brainstorm→...  ← "具体哪一步",可分支、可跳整段
```

判断当前主体流程 = `phaseOf(currentNode)`(节点所在容器,core §6)。约定:阶段单调推进(validate 拒绝倒退);某些任务可经 switch **跳过整段阶段**(如 small 任务从分诊直接进 execute 框,planning 整段不走);每个容器声明一个 `entry` 入口节点(框外指进来只能落在它上,保证阶段单入)。分诊区节点(`phase:null`)在三框之前,不改 task.status。

---

## 2. `ttur next` 门禁

CLI 命令的 agent 面向入口是 `ttur next`,它读取当前任务的 `state.currentNode`,再调用 core 门禁。**skill 节点**走 gate;**switch 节点**不带 branch 时只输出合法分支与下一条命令提示,带 `--branch <label> --reason "..."` 时才推进。`ttur complete <node>` 明确删除,不保留兼容入口;测试和 web 也应调用 `nextNode` 或更小的纯函数,不能重新暴露 node 参数。
...
core.nextNode(scope, taskId, opts?)
  ...
  switch 节点?
    ├─ 未传 opts.branch ──► exit 2:输出合法分支 + nextAction="ttur next --branch <label> --reason ..."
  ...
  skill 节点?(有 gate 才查,全可选)
    ...
    └─ gate.approval 且未 approve? ──► exit 2:"需 ttur approve"
...
export function nextNode(
  ...
  if (g.approval && !isApproved(scope, taskId, nodeId)) return gate(`"${nodeId}" 需 ttur approve`);
...
const branchGate = (m: string, extra: Pick<NextResult, 'branches' | 'nextAction'>): NextResult =>
  ({ ok: false, exitCode: 2, message: m, ...extra });
```
...
> **落地状态**:当前代码已实现旧的 `completeNode` 与 CLI `ttur complete <node>`;本轮决策是迁移为 `nextNode` / `ttur next` 后删除 `complete` 命令与 `completeNode` 命名,不保留兼容入口。

### 2.3 成功输出契约(简单 JSON,面向 agent)

**命令只给 agent 读,输出简单 JSON**(人看进度去 web)。`ttur next` 成功不是干巴巴的 exit 0:stdout 吐一段结构化「下一步」让 agent 自己解析、敲下一条命令——交互模式下 session-start 本会话不会再触发,这段就是 agent 继续推进的线索。**不重复注入上下文**(规范/背景在会话开始已注入一次,§4)。
...
// 门禁未过:告诉 agent 卡在哪
{ "ok": false, "node": "grill-me", "blocked": ["缺产物 design.md(不存在或为空)", "需 ttur approve"] }
...
// 走到终点
{ "ok": true, "done": "wrapup", "next": null, "message": "workflow 完成,可 ttur task archive <id>" }
```
...
- 门禁失败**不改变 state**:停留原节点,修复后再次 `ttur next` 即返工。workflow 图无环(validate 拒绝回边),state 无迭代轮次概念。
...
- 逃生通道:`ttur next --skip --reason "..."`——人工显式跳过(门禁配错/检查 flaky 时用),事件留痕(type=skip)。`--skip` 应在 skill 正文中声明为用户授权后才可使用。
...
```text
harness 推进到 switch → 停下,`ttur next` 输出各分支 criteria(§2.3)
agent 判定 → ttur next --branch <label> --reason "依据"
  ├─ label 非法 / 缺 --branch → exit 2(门禁失败),停下重判
  └─ 合法 → 记 state.decisions[node]={branch,reason,by,at} + decision 事件 → 沿该分支 next 路由
判错了(已进错误分支)→ ttur rewind <switch> 退回重判(§3.1)
```
...
`gate.approval:true` 的节点,完成前需先有人确认。**写确认状态必须由人触发**(agent 或 web 跑 `ttur approve`,系统读取当前 `state.currentNode`,经 `approveCurrentNode` 写入 `state.approvals[currentNode]` 并追加一条 `approval` 审计事件),门禁读它。交互模式下的流转:

```text
agent 干完 → ttur next → 门禁失败:"需 ttur approve"
agent 停下告诉用户:"design.md 待你审,批准请让我 approve;在此之前我不继续"  ← 停,不轮询、不超时
用户(对话里)说同意 → agent 跑 ttur approve(by=当前 .developer.slug,记时间戳,节点=当前 currentNode)
agent 重跑 ttur next → 过 → 推进
```

**取舍点明**:允许 agent 替你跑 `ttur approve`,等于把 approval 从「系统硬门禁」降为「停下+留痕的软约定」——强制 agent 停下问你的只剩 skill 提示词那句话(正是 Tuteur 想取代的软约束)。对单人交互这是合理取舍(人就在对话里),但 approval 不再是机器能挡住 agent 的关卡。配套:不依赖 web(web 仅展示/可选入口)、不轮询不超时(不回就停着)、无过期逻辑(批了一直有效,web 显示时间戳供人察觉);approval 事件诚实记 `by`+时间。
...
`task.status` 由新 `currentNode` 经 `phaseOf(wf, currentNode)` 驱动(planning 框→planning;execute/finish 框→in_progress;`null`→completed;分诊区 `phase:null` 不改 status),写回 `core.writeTask`。validate 强制每个 switch 有且仅有一个 `default`、图无环、阶段单调不倒退。纯函数 + 单测,确定性核心(core.md K4)。原 `evaluateDecision`/`readSignal`/signal 三源模型**已废弃**。

### 3.1 switch 判错:rewind 恢复

switch 判定后已沿分支前进,图无环不能靠回边返工。判错(或想改判)用显式恢复动作 `ttur rewind --to <node>`(留痕)。这里保留目标节点参数,因为回退目标通常不是当前节点,无法仅由 `state.currentNode` 推断。

```text
还停在 switch 没往下走 → 重跑 ttur next --branch <Y> 直接覆盖,重新路由
已进错误分支干了活才发现 → ttur rewind --to <switch>
...
- 内容源是**知识库**(`knowledge/`,knowledge.md):agent 维护的 wiki + index,外加 `kind:template` 产物模板(knowledge §4.1)。注入形态**由条目 `inject` 字段决定**:`index`(缺省,注 title+summary+路径,按需下钻)/ `full`(产物模板、必读短规范等,注正文)。**会话须知(guide.md)不走知识库**——它是工具文件 `.tuteur/guide.md`(§6.4)。
- `context.json` 语义、`resolvePlannedContext` 归 core(core.md §5/§6);**按知识 id 引用、default/node 两层(项目共享,不分用户)**(knowledge.md §7);返回 `PlannedEntry[]`(带 `mode:full|index`)而非裸 id 清单。
...
**① Tuteur 自带工作流 skill(brainstorm/grill-me/dev/check/finish)**——一份 canonical 模板,`init` 铺到**所选每个工具**自己的目录(占位符按平台渲染 + link/copy):

```text
templates/common/skills/<base>/SKILL.md   (含 {{变量}})
   │ resolveWorkflowSkills:替换变量
   ▼
.agent/skill/tuteur-<base>/SKILL.md        (canonical;Codex/Gemini 直接读)
   │ symlink|copy(Claude)
   ▼
.claude/skills/tuteur-<base>/
```

运行时 agent **用自己工具里的那份**:接力 JSON 给出 skill 名,Tuteur 据当前平台(detectPlatform)+ `templateContext.cmdRefPrefix`(Claude `/tuteur:`、Codex `$`)渲染成该工具**原生的调用方式**——**不跨读别的工具目录下的文件**。各工具内容同源(都来自那份模板),所以一致。变量替换表(`{{PRODUCT_NAME}}`/`{{SKILL_NAME}}`/`{{CMD_REF_PREFIX}}`/`{{USER_ACTION_LABEL}}`/`{{CLI_FLAG}}`)来自 `templateContext`。

**② 第三方发现的 skill**(用户 `.codex/skills/foo` 等)——Tuteur 不铺到其他工具,**工具耦合**:引用只在 Codex 有的 skill,这条 workflow 就绑死 Codex,只在该工具下原生使用,同样不跨读。
...
当前缺口:workflow 类 5 个 SKILL.md(brainstorm/grill-me/dev/check/finish)+ 维护类 `tuteur-knowledge`(知识库维护,knowledge.md §5)全是 `TODO:` 占位。门禁可先于 skill 内容落地,但端到端跑通前必须填实(§9 H6)。`tuteur-knowledge` 不是 workflow 节点,而是按需调用的维护 skill,不进默认 workflow。(注:默认 workflow 的分诊 `triage` 是 **switch 节点、不引用 skill**,故无需 classify skill。)
...
| `inject-workflow-state`   | UserPromptSubmit(Gemini 为 `BeforeAgent`) | 每轮用户输入          | **轻量 breadcrumb**:当前节点 + 下一条命令(`ttur next` 或 `ttur next --branch ...`),防止 agent 跑偏当前步                                                                              | `additionalContext` JSON                | P2                    |
...
- **软失败**:任何异常只写 stderr,退出码非 0,平台忽略——**hook 绝不阻断会话**(Tuteur 的硬约束在 `ttur next` 门禁,不在 hook)。
...
会话启动注入的内容分段组织(参考 Trellis `session-start.py` 的 `<session-context>`/`<current-state>`/`<workflow>`/`<task-status>` 分块),`ttur hook session-start` 按 **section 流水线**依次拼出。每段标「来源」:`fixed`=代码常量(协议外壳,极少改);`config`=读文件、**用户可改**;`derived`=core 现算,不可配:

| 段                     | 来源        | 内容                                                                                            | 取自                                         |
| ---------------------- | ----------- | ----------------------------------------------------------------------------------------------- | -------------------------------------------- |
| `<session-context>`    | **fixed**   | 一句话告知「这是 Tuteur 托管项目,按下文走」                                                     | core 常量                                    |
| 首回复确认提示(一次性) | **fixed**   | 让 agent 在首条可见回复确认「Tuteur 已注入」,人能直接看到 hook 生效                             | core 常量                                    |
| `<guide>`              | **config**  | **项目须知 / Tuteur 基本介绍**:读工具文件 `.tuteur/guide.md`,有则注全文                         | `readGuide`(store);**工具文件,非知识库条目** |
...
设计落点(对应你的诉求「文案写死、想让用户改」):**把可变内容从代码挪进 `config` 段**——「Tuteur 介绍/项目须知」放进工具文件 `.tuteur/guide.md`(直接编辑或 web 编辑),「注哪些规范」走 `context.json` + 知识库;`fixed` 段只剩协议外壳。`derived` 段是现算事实(任务情况、git 块均已实现),既不该写死也不该让用户配。

> **落地状态**:`renderSessionStart` 已把 `<guide>`(读 `.tuteur/guide.md`,无则跳过)与 git 块接通;`resolvePlannedContext` 返回带 full/index 形态的 `PlannedEntry[]`,session-start 据形态注正文/索引。section 严格分块(guide → current-state → workflow → task-status 各自成段)为后续细化,当前为单块拼接。

任务状态机(对齐 Trellis 的多态 Next-Action,映射到节点图):

- **NO ACTIVE TASK** → 引导描述需求让 **agent 建任务**(`ttur task create "<title>"`,新任务由 agent 建,core §3.1)
- **AMBIGUOUS**(多个未完成任务、无指针)→ 不猜,列候选,引导 `ttur task start <id>` 选定或新建(harness §7.1)
- **STALE POINTER**(指针指向已不存在的任务)→ 引导清理指针后再 start
- **停在 skill 节点**(PLANNING / IN PROGRESS)→ 注入当前节点 + 推进命令 `ttur next`
- **停在 switch 节点** → 输出各分支 criteria + 判定命令 `ttur next --branch <label> --reason "..."`
...
export function runSessionStartHook(): number {
  ...
  if (r && 'ambiguous' in r) {
    out.push(`- AMBIGUOUS:多个未完成任务 ${r.ambiguous.join(', ')} · Next-Action: ttur task start <id>\n`);
  ...
  if (taskId) {
    ...
    if (node?.type === 'switch')
      out.push(`- Next-Action: ttur next --branch <label> --reason "..." --task ${taskId}\n`);
    else out.push(`- Next-Action: ttur next --task ${taskId}\n`);
  } else {
    out.push('- NO ACTIVE TASK · Next-Action: 描述需求让我 ttur task create "<title>"\n');
  }
...
要点:软失败(异常只记 stderr 不阻断);每次会话点名 `ttur next` 推进入口;注入由 core 统一计算,Codex/Claude/Gemini 共用同一逻辑,差异只在 §6.2 的输出信封。⚠️ Codex 的 hook 需用户手动开 feature flag 并 `/hooks` 信任(cli.md §8.4)。
...
- 内容:`当前任务 · 当前节点 · 主体阶段 · Next-Action: ttur next`;无活跃任务时给「描述需求后 `ttur task create`」的提示。若当前节点是 switch,Next-Action 带 `--branch <label> --reason "..."`。
...
解析优先级(core.resolveCurrentTask):
  ...
  未完成任务有多个、又无指针无 --task → AMBIGUOUS:不猜,报告用户、列候选,
    引导 `ttur task start <id>` 选定或新建任务,再继续;绝不随便挑一个推进(挑错比报错更糟)
```
...
- **上下文供给走 pull-based prelude**(MVP 唯一路径):子 agent 定义顶部写「Required: 先读 `ttur task status` 与任务上下文清单(implement.json 等)」,由子 agent 自己拉取。支持工具派发 hook 的平台(Claude 等)的 `inject-subagent-context` 自动注入留 P1+(§6.1)。
...
| 扩展             | 改什么                                                                            | 生效                           | 安全网                               |
| ---------------- | --------------------------------------------------------------------------------- | ------------------------------ | ------------------------------------ |
| 自定义 workflow  | 加 `workflows/<id>.workflow.json` + task 指定 `workflow`                          | core 按新定义门禁              | `workflow validate` 校验 skill/路径  |
| 自定义 skill     | 改/加 `.agent/skill/<name>/SKILL.md`                                              | `skill` 字段引用               | `update` 哈希保护用户改动(cli.md §7) |
...
- 旧门禁路径(`completeNode`:skill gate / switch `--branch`)、`advanceWorkflow`、`rewindTo`、`approveNode`、`resolveCurrentTask`、`ttur hook session-start` 已落地并跑通闭环;**剩余缺口是迁移为 `ttur next`/`nextNode`、删除 `complete`、5 个 SKILL.md 正文仍为 TODO**(端到端体验差最后一环),以及 per-turn / 子 agent hook、平台信封适配未做。
...
| #   | 项                                                                                                                                     | 状态/优先级 | 依赖                    |
| --- | -------------------------------------------------------------------------------------------------------------------------------------- | ----------- | ----------------------- |
| H1  | 默认 workflow 改固定阶段容器 + 节点图(triage switch 分诊)+ gate 门禁                                                                   | ✅ 已实现 | core §4.3               |
| H2  | `advanceWorkflow`(沿 next、遇 switch 停、无环/阶段单调校验)+ `rewindTo` + 单测                                                         | ✅ 已实现 | core K4                 |
| H3  | 旧 `completeNode` 门禁(skill 走 gate / switch 带 --branch;失败不改 state;appendEvent)                                                   | ✅ 已实现(待迁移命名) | H2                      |
| H3a | `ttur next` / `nextNode`:读取当前节点、承载唯一推进门禁、switch 无 branch 时输出分支提示,并删除 `complete`                              | P0     | H3                      |
| H4  | `ttur hook session-start`(分段注入 + 多态 Next-Action 含 AMBIGUOUS,§6.4)+ hook 声明文件(命令直配)                                      | ✅ 已实现(session-start) | core context            |
...
| H6  | 填实 5 个 SKILL.md(brainstorm/grill-me/dev/check/finish;含子 agent 调度协议)                                                           | P0     | ——                      |
| H7  | `ttur task start` + resolveCurrentTask(指针/兜底/STALE/AMBIGUOUS)                                                                      | ✅ 已实现 | §7.1                    |
| H8  | `next` 成功接力 JSON + switch 接力(分支 criteria,§2.3)                                                                                 | P0     | §2.3                    |
| H9  | switch 判定(--branch/--reason 校验、记 state.decisions/事件)+ `ttur rewind`                                                            | ✅ 已实现 | §2.5、§3.1              |
| H10 | skill 解析与 validate(连通/无环/阶段单调/switch default/skill 存在)                                                                    | ✅ 已实现 | core §5.1               |
| H11 | approval 读写(`state.approvals[currentNode]` + approval 事件,`ttur approve`,§2.6;删除 approve 的 node 参数)                            | 🟡 旧 `approve <node>` 已实现;需迁移为当前节点 | web §6                  |
| H12 | `inject-workflow-state`(每轮 breadcrumb,§6.5)/ `inject-subagent-context`(子 agent 注入,§6.6;push 平台)hook                             | P2     | §6.1、§7.2              |
| H13 | `renderSessionStart` 拆 section 流水线(fixed/config/derived,§6.4)+ 补 `<current-state>` git 块 + 接通 `<guide>`(读 `.tuteur/guide.md`) | ✅ 已实现 | core §6                 |
