...
export function completeNode(
  ...
  if (g.approval && !isApproved(scope, taskId, nodeId)) return gate(`"${nodeId}" 需 ttur approve ${nodeId}`);
...
export function runSessionStartHook(): number {
  ...
  if (taskId) {
    ...
    else out.push(`- Next-Action: ttur complete ${st.currentNode} --task ${taskId}\n`);
  } else {
