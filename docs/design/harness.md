...
```text
skill 节点   单出(next),入度不限   读 skill 照做,受可选 gate 约束;做完 ttur complete <node> 推进
switch 节点  单入多出(branches)     岔路口:agent 判断走哪条;走到就停,ttur complete <node> --branch;必含一个 default
...
CLI 命令(`ttur complete <node>`)与 web 端点共用 `core.completeNode`。**skill 节点**走门禁(gate);**switch 节点**也用 `complete`,但需带 `--branch <label> --reason "..."`(agent 判定结果,§2.5)。

### 2.1 流程

```text
core.completeNode(scope, taskId, node, opts?)
  读 workflow + state
  node == state.currentNode? ──否──► exit 2(喊错节点,记事件=跳步证据)
  switch 节点?
  ...
  skill 节点?(有 gate 才查,全可选)
    ...
    └─ gate.approval 且未 approve? ──► exit 2:"需 ttur approve <node>"
...
```ts
export function completeNode(scope: Scope, taskId: string, nodeId: string, opts?: { branch?: string; reason?: string; by?: string }): CompleteResult {
  ...
  if (g.approval && !isApproved(scope, taskId, nodeId))
    return gate(`"${nodeId}" 需 ttur approve ${nodeId}`);
...
**命令只给 agent 读,输出简单 JSON**(人看进度去 web)。`ttur complete` 成功不是干巴巴的 exit 0:stdout 吐一段结构化「下一步」让 agent 自己解析、敲下一条命令——交互模式下 session-start 本会话不会再触发,这段就是 agent 继续推进的线索。**不重复注入上下文**(规范/背景在会话开始已注入一次,§4)。
...
// 门禁未过:告诉 agent 卡在哪
{ "ok": false, "node": "grill-me", "blocked": ["缺产物 design.md(不存在或为空)", "需 ttur approve grill-me"] }
...
```text
harness 推进到 switch → 停下,接力 JSON 输出各分支 criteria(§2.3)
agent 判定 → ttur complete <switch> --branch <label> --reason "依据"
...
`gate.approval:true` 的节点,完成前需先有人确认。**写确认状态必须由人触发**(agent 或 web 跑 `ttur approve <node>`,经 `approveNode` 写入 `state.approvals` 并追加一条 `approval` 审计事件),门禁读它。交互模式下的流转:

```text
agent 干完 → ttur complete grill-me → 门禁失败:"需 ttur approve grill-me"
agent 停下告诉用户:"design.md 待你审,批准请让我 approve;在此之前我不继续"  ← 停,不轮询、不超时
用户(对话里)说同意 → agent 跑 ttur approve grill-me(by=当前 .developer.slug,记时间戳)
agent 重跑 ttur complete grill-me → 过 → 推进
```
...
完成当前节点后产出新 state。**没有自动求值、没有零成本穿越**:skill 沿 `next` 走一步;switch 由 `completeNode` 传入已判定的 branch,沿该分支 `next` 走一步。落到的下一个节点(无论 skill 还是 switch)就是新 `currentNode`,**到 switch 也是停**(等 agent 判定),到 `null` 即完成:

```ts
// 纯函数;switch 需传入已判定的 branch label(由 completeNode 校验后给)
...
switch 判定后已沿分支前进,图无环不能靠回边返工。判错(或想改判)用显式恢复动作 `ttur rewind <node>`(留痕):

```text
还停在 switch 没往下走 → 重跑 ttur complete <switch> --branch <Y> 直接覆盖,重新路由
已进错误分支干了活才发现 → ttur rewind <switch>
...
| `<task-status>` | **derived** | **任务状态机**(下方多态),每态点名显式 Next-Action | `resolveCurrentTask`+`completeNode` 预判 |
...
- **停在 skill 节点**(PLANNING / IN PROGRESS)→ 注入当前节点 + 完成命令 `ttur complete <node>`
- **停在 switch 节点** → 输出各分支 criteria + 判定命令 `ttur complete <node> --branch <label> --reason "..."`
...
export function runSessionStartHook(): number {
  ...
  if (taskId) {
    ...
    if (node?.type === 'switch')
      out.push(`- Next-Action: ttur complete ${st.currentNode} --branch <label> --reason "..." --task ${taskId}\n`);
    else
      out.push(`- Next-Action: ttur complete ${st.currentNode} --task ${taskId}\n`);
  } else {
...
- 门禁(`completeNode`:skill gate / switch `--branch`)、`advanceWorkflow`、`rewindTo`、`approveNode`、`resolveCurrentTask`、`ttur hook session-start` 已落地并跑通闭环;**剩余缺口是 5 个 SKILL.md 正文仍为 TODO**(端到端体验差最后一环),以及 per-turn / 子 agent hook、平台信封适配未做。
...
| # | 项 | 优先级 | 依赖 |
| --- | --- | --- | --- |
| H1 | 默认 workflow 改固定阶段容器 + 节点图(triage switch 分诊)+ gate 门禁 | P0 | core §4.3 |
| H2 | `advanceWorkflow`(沿 next、遇 switch 停、无环/阶段单调校验)+ `rewindTo` + 单测 | P0 | core K4 |
| H3 | `completeNode` 门禁(skill 走 gate / switch 带 --branch;失败不改 state;appendEvent) | P0 | H2 |
| H4 | `ttur hook session-start`(分段注入 + 多态 Next-Action 含 AMBIGUOUS,§6.4)+ hook 声明文件(命令直配) | P0 | core context |
...
| H7 | `ttur task start` + resolveCurrentTask(指针/兜底/STALE/AMBIGUOUS) | P0 | §7.1 |
| H8 | complete 成功接力 JSON + switch 接力(分支 criteria,§2.3) | P0 | §2.3 |
| H9 | switch 判定(--branch/--reason 校验、记 state.decisions/事件)+ `ttur rewind` | P0 | §2.5、§3.1 |
| H10 | skill 解析与 validate(连通/无环/阶段单调/switch default/skill 存在) | P1 | core §5.1 |
| H11 | approval 读写(`state.approvals` + approval 事件,`ttur approve`,§2.6) | P1 | web §6 |
| H12 | `inject-workflow-state`(每轮 breadcrumb,§6.5)/ `inject-subagent-context`(子 agent 注入,§6.6;push 平台)hook | P2 | §6.1、§7.2 |
| H13 | `renderSessionStart` 拆 section 流水线(fixed/config/derived,§6.4)+ 补 `<current-state>` git 块 + 接通 `<guide>`(读 `.tuteur/guide.md`) | P0 | core §6 |
| H14 | `resolvePlannedContext` 返回 `PlannedEntry[]`(带 `mode:full\|index`)+ session-start 按形态注正文/索引 | P0 | core §4.5、knowledge §7 |
