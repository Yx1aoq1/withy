...
> 主线:**唯一逻辑层 `@tuteur/core` + 唯一入口 `ttur` + hook 薄转发 + 双层(全局/项目)+ InitConfig 统一初始化 + 注册表/per-agent configurator/通用层 + 固定阶段容器(规划/执行/收尾)workflow(skill/switch 两类节点)+ `ttur next` 低歧义推进 + switch 靠 agent 判断(报分支、harness 路由)。**
